  ...
  /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataDirectory == null) {
            ...
            filePath = "dataDirectory/list.txt";
        }
        CommentHandler handler = new FileHandler(filePath);

        ListIterator<Comment> listIterator = handler.getComments().listIterator(handler.getComments().size());
        List<Comment> newList = new ArrayList<>();
        while (listIterator.hasPrevious()) {
            newList.add(listIterator.previous());
        }

        request.getSession().setAttribute("comments", newList);
